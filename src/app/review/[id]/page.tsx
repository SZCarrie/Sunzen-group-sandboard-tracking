...
import { addReview } from "@/app/admin/documents/[id]/actions";

type DocStatus = "draft" | "submitted" | "in_review" | "needs_revision" | "approved";

// walks the owner's manager_id chain looking for the viewer, so the Add Review form only
// shows for an actual manager (direct or skip-level) — not for someone who can merely read
// the document via admin oversight (RLS's has_group_oversight bypass covers that case too,
// but oversight alone shouldn't grant review authority here).
async function isManagerOf(
  ...
  for (let hop = 0; hop < 8 && currentId; hop++) {
    const { data: person }: { data: { manager_id: string | null } | null } = await supabase
      .from("profiles")
      .select("manager_id")
      .eq("id", currentId)
      .maybeSingle();
    if (!person?.manager_id) return false;
    if (person.manager_id === viewerId) return true;
    currentId = person.manager_id;
  }
  return false;
}
...
export default async function ReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = await isManagerOf(supabase, user.id, doc.owner_id);
  ...
  return (
    <div>
      <Link href="/review" className="mb-4 inline-block text-xs text-ink-soft underline decoration-line underline-offset-4 hover:text-ink">
        ← {tr.backToList}
      </Link>
      ...
      <div className="mb-6 rounded-md border border-line bg-paper-raised p-4 text-sm">
        <p>
          ...
          {owner?.full_name} ({owner?.email}) · {owner ? getRoleLabel(locale, owner.role) : ""}
        </p>
        <p className="mt-1">
